...
title: "Publications"
toc: true
image: assets/img/nature-paper.png
listing:
  - id: featured
...
lab <- c("Gabrielle Koerich", "Andrea Tabi", "Holly A. L. Harris")
...
# outcomes <- Publications %>%  janitor::tabyl(CATEGORY) %>% data.frame %>% pull(n, CATEGORY) 
# #outcomes_with_types <- Publications %>% dplyr::mutate(field = ifelse(is.na(JCR), CATEGORY ,JCR)) %>% janitor::tabyl(field) %>% data.frame %>% pull(n,field) 
...
#   ds <- pubdf %>% filter(bibtype == pub_types[p])
